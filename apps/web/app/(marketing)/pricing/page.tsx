...
const ctaClasses: Record<Plan['ctaStyle'], string> = {
  ...
  secondary: 'bg-[var(--bg-surface)] border border-[var(--border)] hover:bg-[var(--bg-subtle)] text-[var(--text-1)] font-semibold',
  ghost: 'bg-[var(--bg-surface)] border border-brand-500 hover:bg-brand-50 dark:hover:bg-[rgba(16,185,129,0.1)] text-brand-600 font-semibold',
};
  ...
  return (
    <div className="py-16 px-12">
      ...
      <div className="text-center mb-10">
        <h1
          className="font-extrabold tracking-[-0.03em] text-[var(--text-1)] mb-3"
          style={{ fontSize: '40px' }}
        >
          Harga jelas. Tumbuh sesuai skala bisnismu.
        </h1>
        <p className="text-sm text-[var(--text-2)]">Mulai gratis, upgrade kapan saja.</p>
      </div>

      <div className="flex items-center justify-center gap-1 mb-10 bg-[var(--bg-subtle)] w-fit mx-auto rounded-full p-1">
        <button
          onClick={() => setYearly(false)}
          className={`rounded-full font-semibold px-4 py-1.5 text-sm transition-colors cursor-pointer ${
            !yearly
              ? 'bg-[var(--bg-surface)] shadow-sm text-[var(--text-1)]'
              : 'text-[var(--text-2)]'
          }`}
        >
          Bulanan
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3.5 max-w-[1000px] mx-auto">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.name}
              className={`bg-[var(--bg-surface)] border rounded-[20px] p-5 flex flex-col relative ${
                plan.popular
