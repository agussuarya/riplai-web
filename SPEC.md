...
| T4 | x | docker infra — pin next→^16/turbo→^2, rm .nvmrc+.node-version, add standalone output to 3 apps, Dockerfile (ARG APP+ENV multi-stage), docker-compose.yml (3 services 3001-3003), .env.local/.env.staging/.env.production, .dockerignore, Makefile (ENV=local|staging|production) | V7,V8,I.web,I.partner,I.admin | 2026-05-16-docker-infra-setup |
